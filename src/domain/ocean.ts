...
export const OCEAN_RUN_CHAPTERS: OceanRunChapter[] = [
  { id: "beach", number: "01", title: "해변", mode: "보드를 들고 달리기", hazards: "야자수 · 유목 · 바위", requiredItemId: null },
  { id: "surf", number: "02", title: "파도", mode: "서핑보드 라이딩", hazards: "물고기 떼 · 상어 · 큰 파도", requiredItemId: null },
...
export const OCEAN_GAME_BY_ID = new Map<MiniGameId, OceanGameInfo>([[OCEAN_RUN_GAME.id, OCEAN_RUN_GAME]]);
...
export function oceanZoneForGame(id: MiniGameId): OceanZoneId | null {
  return id === OCEAN_RUN_GAME.id ? "beach" : null;
}
...
export function oceanCompletionCopy(result: MiniGameResult): string | null {
  if (result.gameId !== "ocean-run") return null;
  return result.success ? "해변부터 심해까지 Ocean Run 완주!" : "Ocean Run 탐험 기록을 저장했어요.";
}
