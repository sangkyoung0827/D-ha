...
export function MiniGameOverlay({ id, result, debug, onClaim, onExit }: { id: MiniGameResult["gameId"]; result: MiniGameResult | null; debug: boolean; onClaim(result: MiniGameResult): void; onExit(): void }) {
  ...
  if (result) {
    ...
    const oceanGate = id === "ocean-run" && !result.success && result.durationMs >= 23_000
      ? (inventory["ocean-oxygen-tank"] ?? 0) < 1
        ...
        : result.durationMs >= 35_000 && (inventory["ocean-submarine"] ?? 0) < 1
          ...
          : null
      : null;
    const resultTitle = id === "ocean-run" ? result.success ? "심해 탐험 완주!" : oceanGate?.title ?? "탐험 기록을 세웠어요" : result.success ? "멋진 항해였어요!" : "다음 해류에서 다시 만나요";
    return <div className="minigame-result" role="dialog" aria-modal="true" aria-labelledby="result-title"><div className="result-medal">{oceanGate?.icon ?? (result.success ? "✦" : "≈")}</div><p className="eyebrow">{id === "ocean-run" ? "OCEAN RUN COMPLETE" : "CURRENT COMPLETE"}</p><h2 id="result-title">{resultTitle}</h2><strong>{result.score.toLocaleString()}점</strong>{oceanGate && <div className="ocean-gate-result"><strong>{oceanGate.copy}</strong><small>보상을 받은 뒤 Ocean 상점을 열어 준비할 수 있어요.</small></div>}<div className="reward-row"><span>● {reward.coins} 코인</span><span>↑ {reward.xp} XP</span></div><button className="primary-button wide" data-testid="claim-reward" onClick={() => onClaim(result)}>{isOceanGame(id) ? "보상 받고 바다로" : "보상 받고 게임룸으로"}</button></div>;
  }
  const laneGame = id === "current-run" || id === "ocean-run" || id === "reef-surf" || id === "deepsea-descent";
  const actionLabel = id === "ocean-run" ? "점프 / 상승" : id === "beach-pingpong" ? "탁구공 치기" : id === "beach-volleyball" ? "공 받기" : id === "beach-football" ? "슛" : null;
  return <aside className="minigame-controls" aria-label="미니게임 접근성 컨트롤"><div><p className="eyebrow">PLAYING</p><strong>{definition?.title ?? "게임 준비 중"}</strong>{id === "ocean-run" && liveScore && <small className="runner-live-chip">CH {liveScore.playerPoints} · {liveScore.computerPoints} m</small>}{liveScore && <span className="sr-only" role="status" data-testid="minigame-live-score">{id === "ocean-run" ? `챕터 ${liveScore.playerPoints}, 거리 ${liveScore.computerPoints}미터, 점수 ${liveScore.score}` : `내 점수 ${liveScore.score}, 나 ${liveScore.playerPoints}, 컴퓨터 ${liveScore.computerPoints}`}</span>}</div><div className="control-buttons">{laneGame && <><button onClick={() => gameBridge.emit("minigame:move", { direction: -1 })} aria-label="왼쪽 레인 이동">←</button><button onClick={() => gameBridge.emit("minigame:move", { direction: 1 })} aria-label="오른쪽 레인 이동">→</button></>}{actionLabel && <button className="minigame-action" data-testid="minigame-action" onClick={() => gameBridge.emit("minigame:action", undefined)}>{actionLabel}</button>}<button onClick={() => { gameBridge.emit(paused ? "minigame:resume" : "minigame:pause", undefined); setPaused(!paused); }}>{paused ? "계속" : "일시정지"}</button><button onClick={() => gameBridge.emit("minigame:restart", undefined)}>다시</button>{debug && <button onClick={() => gameBridge.emit("minigame:demo-finish", undefined)}>데모 완료</button>}<button onClick={onExit}>나가기</button></div></aside>;
}
