...
async function createKeeper(page: Page, name = "마루") {
  ...
  await page.getByRole("button", { name: /스튜디오 입장/ }).click();
...
test("캐릭터 생성부터 돌봄, 미니게임, 구매, 장착과 새로고침 저장까지", async ({ page }) => {
  ...
  if (!(await page.getByRole("dialog").isVisible())) {
    await page.getByRole("button", { name: "옷장 열기" }).click();
  }
...
test("데모 시간 경과와 모바일 가로 오버플로를 검증한다", async ({ page }) => {
  ...
  const before = await page.locator(".need-pill").first().getAttribute("aria-label");
  await page.getByText("DEV", { exact: true }).click();
  await page.getByTestId("advance-1h").click();
  const after = await page.locator(".need-pill").first().getAttribute("aria-label");
